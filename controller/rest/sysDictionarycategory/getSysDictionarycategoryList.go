...
// 请求
type getSysDictionarycategoryListRequest struct {
...
// 方法
func GetSysDictionarycategoryList(c *gin.Context) {
	defer func() {
		if err := recover(); err != nil {
			c.JSON(http.StatusOK, model.Response{
				Code:    "sysDictionarycategory.GetSysDictionarycategoryList.ex",
	...
	//region 解析请求参数
	var err error
	var request getSysDictionarycategoryListRequest
	//endregion
	...
	if golibs.Length(request.pageIndex) > 0 {
		if !golibs.IsNumber(request.pageIndex) {
			c.JSON(http.StatusOK, model.Response{
				Code:    "sysDictionarycategory.GetSysDictionarycategoryList.pageIndex is number",
		...
		if err != nil {
			c.JSON(http.StatusOK, model.Response{
				Code:    "sysDictionarycategory.GetSysDictionarycategoryList.pageIndex parse err",
		...
		if request.pageIndexInt < 1 || request.pageIndexInt > 100000 {
			c.JSON(http.StatusOK, model.Response{
				Code:    "sysDictionarycategory.GetSysDictionarycategoryList.pageIndex value err",
	...
	if golibs.Length(request.pageSize) > 0 {
		if !golibs.IsNumber(request.pageSize) {
			c.JSON(http.StatusOK, model.Response{
				Code:    "sysDictionarycategory.GetSysDictionarycategoryList.pageSize is number",
		...
		if err != nil {
			c.JSON(http.StatusOK, model.Response{
				Code:    "sysDictionarycategory.GetSysDictionarycategoryList.pageSize parse err",
		...
		if request.pageSizeInt < 1 || request.pageSizeInt > 500 {
			c.JSON(http.StatusOK, model.Response{
				Code:    "sysDictionarycategory.GetSysDictionarycategoryList.pageSize value err",
	...
	//region 查询【字典分类】列表
	list, total, err := service.GetSysDictionarycategoryList(request.pageIndexInt, request.pageSizeInt)
	if err != nil {
		c.JSON(http.StatusOK, model.Response{
			Code:    "sysDictionarycategory.GetSysDictionarycategoryList.query err",
	...
	//region 组装【字典分类】列表
	sysDictionarycategorysArray := make([]map[string]interface{}, len(list))
	if len(list) > 0 {
		for i, v := range list {
			sysDictionarycategorysArray[i] = map[string]interface{}{
	...
	c.JSON(http.StatusOK, model.Response{
		...
		Data: map[string]interface{}{
			"total": total,
			"list":  sysDictionarycategorysArray,
		},
	})
	//endregion
}
